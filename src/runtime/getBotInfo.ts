import { IBotInfo } from './types/bot'

const REGEX_BOTS = new Map([
  ['google', /[Gg]ooglebot/],
...
export default function (userAgent: string): IBotInfo {
  if (!userAgent) {
    return {
      isBot: false
    }
  ...
  for (const [botName, botPattern] of REGEX_BOTS.entries()) {
